...
export default function Calendar({ jsonData, year, month }) {
    ...
    return(
        <div
            className="grid grid-cols-7 gap-px bg-slate-200 dark:bg-gray-700 rounded-2xl overflow-hidden shadow-2xl max-w-6xl mx-auto border border-slate-200 dark:border-gray-700"
            style={{ zoom: 0.75 }}
        >
            {/* Cabecera Días */}
            {['Lun', 'Mar', 'Mie', 'Jue', 'Vie', 'Sáb', 'Dom'].map((d) => (
                ...
                {[...Array(daysInMonth)].map((_, i) => {
                    ...
                    const isToday = 
                        day === new Date(2026, 4, 18).getDate() && 
                        month === new Date(2026, 4, 18).getMonth() && 
                        year === new Date(2026, 4, 18).getFullYear();
